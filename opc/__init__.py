...
class _OPC(object):
    ...
    def _8bit_unsigned(self, byte):
        """Returns unsigned int from byte

        :param byte: unsigned 8bit integer

        :type byte: byte

        :rtype: 8-bit unsigned int
        """
    ...
    def _calculate_temp_R1(self, LSB, MSB):
        """Returns the combined LSB and MSB
        ...
        return -45 + 175 * number / 65535

    def _calculate_hum_R1(self, LSB, MSB):
        """Returns the combined LSB and MSB
...
class OPCR1(_OPC):
    """Create an instance of the Alphasene OPC-R1. Currently supported by firmware
    versions X-X. opc.OPCR1 inherits from the opc.OPC parent class.
    ...
    def off(self):
        ...
        b1 = self.cnxn.xfer([0x03])[0]          # send the command byte
        sleep(9e-3)                             # sleep for 9 ms
        b2 = self.cnxn.xfer([0x00])[0]          # send the following two bytes
        sleep(0.1)
    ...
    def histogram(self, number_concentration=True):
        ...
        if resp[0] == 0xf3:
            ...
            data['Temperature']     = self._calculate_temp_R1(resp[40], resp[41])
            data['Relative humidity']= self._calculate_hum_R1(resp[42], resp[43])

    #        data['Pressure']        = self._calculate_pressure(resp[40:44])
            data['Sampling Period'] = self._calculate_float(resp[44:48])

            data['Reject count Glitch'] = self._8bit_unsigned(resp[48])
            data['Reject count Long'] = self._8bit_unsigned(resp[49])
            ...
            # Check that checksum and the least significant bits of the sum of histogram bins
            # are equivilant
            if self.calc_crc(resp[0:62]) != data['Checksum']:
                logger.warning("Data transfer was incomplete")
                # return None
    ...
    def toggle_laser(self, state):
        ...
        # If state is true, turn the laser ON, else OFF
        if state:
            b = self.cnxn.xfer([0x02])[0] # 10
        else:
            b = self.cnxn.xfer([0x03])[0] # 11
    ...
    def toggle_fan(self, state):
        ...
        # If state is true, turn the fan ON, else OFF
        if state:
            b = self.cnxn.xfer([0x04])[0] # 100
        else:
            b = self.cnxn.xfer([0x05])[0] # 101

        sleep(0.1)

        return True if a == 0xF3 and b == 0x03 else False

    def toggle_Peripheral(self, state):
        """Toggle the power state of the fan.
    ...
    def read_firmware(self):
        """Read the firmware version of the OPC-R1.
    ...
    def pm(self):
        ...
        **NOTE: This method is supported by firmware v18+.**
        ...
        data['PM1']     = self._calculate_float(resp[0:4])
        data['PM2.5']   = self._calculate_float(resp[4:8])
        data['PM10']    = self._calculate_float(resp[8:12])
        data['Checksum']    = self._16bit_unsigned(resp[12],resp[13])
